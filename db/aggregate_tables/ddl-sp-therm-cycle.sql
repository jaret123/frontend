...
CREATE PROCEDURE sp_therm_cycle (IN refresh_mode varchar(10), IN record_id int)
...
BEGIN
	...
	ELSEIF refresh_mode = 'INCR' THEN 
		SELECT @max_id:=max(dai_meter_actual_id) FROM xeros_therm_cycle;
		SET where_clause = CONCAT(' dai_meter_actual_id > ' , @max_id , ' ');

	ELSEIF refresh_mode = 'DIFF' THEN

		SET where_clause = ' dai_meter_actual_id NOT IN (SELECT dai_meter_actual_id FROM xeros_therm_cycle) ';

	ELSEIF refresh_mode = 'SINGLE' THEN
		
		DELETE FROM xeros_therm_cycle WHERE dai_meter_actual_id = record_id;
